...
def plot_training(file_name):
    ...
    plt.tight_layout()
    plt.savefig(f'{os.path.dirname(file_name)}/Training and Validation.png', dpi=300)
    # plt.savefig(f'{os.path.dirname(file_name)}/Training and Validation.png', dpi=300, transparent=True)
...
def plot_testing(file_name):
    ...
    test_directory = os.path.dirname(file_name)
...
def plot_PR_curve(model_name, y_test, y_pred_pro, target_names, dataset_folder):
    ...
    plt.legend()
    plt.savefig(os.path.join(dataset_folder, f'{model_name}_pr.png'), dpi=300)
...
def plot_ROC_curve(model_name, y_test, y_pred_pro, target_names, dataset_folder):
    ...
    plt.legend()
    plt.savefig(os.path.join(dataset_folder, f'{model_name}_roc.png'), dpi=300)
    plt.show()
...
def plot_confusion_matrix(model_name, y_test, y_pred, target_names, dataset_folder):
    ...
    plt.savefig(os.path.join(dataset_folder, f'{model_name}_cm.png'), dpi=300)
    # plt.show()
...
#
#
#     def _plot_tsne(self, name, labels, save_path=None):
#         file_path = self._get_file_path(f'{name}.npz')
#         # Load the reduced_data from the .npz file
#         reduced_data = np.loadtxt(file_path, delimiter=',')
#         # import pickle
#         # with open(file_path, 'rb') as file:
#         #     reduced_data = pickle.load(file)
#
#         # Set a Seaborn style
...
#         sns.scatterplot(x=reduced_data[:, 0], y=reduced_data[:, 1], hue=labels, palette=custom_palette, s=50, alpha=0.7)
#
#         # Add title and axis labels
#         plt.title(f"{name} layer t-SNE Plot")
